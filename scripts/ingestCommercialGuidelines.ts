...
const BATCH = 100;
// Written after a successful upsert and committed. A corpus file that changes
// without a re-ingest leaves the index serving stale text, which is invisible
// at runtime — the agent just answers from a document that no longer matches
// the repo. `corpusManifest.test.ts` compares this to the working tree so that
// drift fails a test run instead of quietly degrading answers.
...
export function parseFile(filePath: string): ParsedDoc {
  ...
  return {
    ...
    // Relative to the corpus root ("muscle/cervical-laminectomy.md"). An
    // absolute path records whichever machine ran the ingest, so the column
    // churned on every run from a different checkout and pinned one
    // developer's home directory into the table.
    sourcePath: path.relative(DATA_DIR, filePath).replace(/\\/g, "/"),
  };
}

export function embeddingText(d: ParsedDoc): string {
...
/** Content hash of every corpus file, keyed by its path relative to DATA_DIR. */
export function hashCorpus(files: string[]): Record<string, string> {
...
function writeManifest(files: string[]): void {
  const manifest = {
    // Regenerated by `yarn ingest:commercial`; do not hand-edit.
    ingestedAt: new Date().toISOString(),
    count: files.length,
    files: hashCorpus(files),
  };
  fs.writeFileSync(MANIFEST_PATH, `${JSON.stringify(manifest, null, 2)}\n`);
  console.log(`[ingest] Manifest written: ${MANIFEST_PATH}`);
}
...
async function main() {
  ...
  for (let i = 0; i < unique.length; i += BATCH) {
    ...
    // Uncached on purpose — see embedMany. The index is the durable store, so
    // a re-ingest should recompute rather than replay whatever the cache holds.
    const vectors = await embedMany(slice.map(embeddingText), { useCache: false });
