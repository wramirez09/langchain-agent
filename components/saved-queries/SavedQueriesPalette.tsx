...
const DETERMINATION_BADGE: Record<Determination, string> = {
  meets_criteria: 'bg-success/10 border-success/30 text-success',
  conditional: 'bg-warning/10 border-warning/30 text-warning',
  more_info_needed: 'bg-warning/10 border-warning/30 text-warning',
  likely_denial: 'bg-destructive/10 border-destructive/30 text-destructive',
  not_supported: 'bg-destructive/10 border-destructive/30 text-destructive',
}
...
export function SavedQueriesPalette({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="top-[18%] w-[calc(100%-2rem)] max-w-2xl translate-y-0 gap-0 overflow-hidden rounded-2xl border border-border p-0 sm:w-[calc(100%-3rem)]">
        <DialogTitle className="sr-only">Saved Queries</DialogTitle>

        {pendingSave ? (
          <div className="flex items-center justify-between gap-3 border-b border-warning/20 bg-warning/10 px-4 py-2.5">
        ...
        <Command className="rounded-2xl border-0 bg-card shadow-none">
          <CommandInput
            placeholder="Search saved queries…"
            className="pr-10 text-sm placeholder:text-faint"
          />
          <CommandList className="max-h-[420px]">
            {isLoading ? (
              ...
              <div className="px-6 py-10 text-center text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground-soft">Save</span> to
                keep it here.
              </div>
            ) : (
              <>
            ...
            <div className="flex items-center gap-4">
              {queries.length > 0 ? (
                <button
                  ...
                  className="font-medium text-faint transition-colors hover:text-destructive"
                >
                  Clear all
                </button>
              ) : null}
  ...
  return (
    <CommandItem
      ...
      value={[
        ...
        .join(' ')}
      onSelect={onReapply}
      className="group cursor-pointer items-start gap-2.5 rounded-lg px-2.5 py-2.5 data-[selected=true]:bg-muted"
    >
      <div className="min-w-0 flex-1">
        ...
        {detail ? (
          <p className="mt-1 truncate text-xs leading-snug text-foreground-soft">
            {detail}
          </p>
        ) : null}
        <div className="mt-1 flex min-w-0 items-center gap-2 text-[11px] text-muted-foreground">
          <span
            className={cn(
              ...
              q.origin === 'form'
                ? 'border-primary/10 bg-primary/5 text-blue-600'
      ...
      <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 group-data-[selected=true]:opacity-100">
        <button
          ...
          className={cn(
            ...
            q.pinned
              ...
              : 'text-faint hover:text-blue-600',
          )}
        >
          <Pin
        ...
        <button
          ...
          title="Delete"
          className="grid h-7 w-7 place-items-center rounded-md text-faint transition-colors hover:text-destructive"
        >
          {busy ? (
