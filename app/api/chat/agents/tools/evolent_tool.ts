    ...
    try {
      ...
      const ftsResult = await withRetry(
        ...
        },
        {
          ...
          context: "Evolent full-text search",
          onRetry: (attempt, error) => {
            console.warn(`⚠️ [EvolentSearchTool] FTS retry ${attempt}:`, error.message);
          }
        }
      );
      ...
      if (!ftsResult.success || !ftsResult.data) {
        const errorInfo = trackRetryError(
          ...
          "Evolent full-text search",
          ftsResult.attempts,
          undefined,
          "evolent-fts-search"
        );
        console.error("[EvolentSearchTool] FTS error:", errorInfo);
      } else {
...
Please include more context such as the procedure name or codes.`;
      ...
      const fuzzyResult = await withRetry(
        ...
        {
          ...RETRY_CONFIGS.DATABASE,
          context: "Evolent fuzzy search",
          onRetry: (attempt, error) => {
            console.warn(`⚠️ [EvolentSearchTool] Fuzzy search retry ${attempt}:`, error.message);
          }
        }
      );
      ...
      if (!fuzzyResult.success || !fuzzyResult.data) {
        const errorInfo = trackRetryError(
          ...
          "Evolent fuzzy search",
          fuzzyResult.attempts,
          undefined,
          "evolent-fuzzy-search"
        );
        console.error("[EvolentSearchTool] Fuzzy error:", errorInfo);
        return `No Evolent guidelines found related to **"${query}"** after multiple attempts. Please try again later.`;
      }
