...
export function resolveStateId(input: string): number | null {
  const trimmed = input.trim();
  const upper = trimmed.toUpperCase();

  // Try abbreviation lookup first (CA, TX, NY, etc.)
  const fullName = STATE_ABBREV[upper];
  if (fullName) {
    const match = data.find(item =>
      item.description.toLowerCase() === fullName.toLowerCase() ||
      item.description.toLowerCase().includes(fullName.toLowerCase() + ' - entire') ||
      item.description.toLowerCase().startsWith(fullName.toLowerCase())
    );
    return match?.state_id ?? null;
  }

  // Full name or substring match (e.g., "California", "california - entire state")
  const lower = trimmed.toLowerCase();
  const match = data.find(item =>
    item.description.toLowerCase() === lower ||
